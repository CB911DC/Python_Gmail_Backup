...
class emailBackup(object):
    ...
    def get_mail_items(self, server, passwd):
        """Retrieves number of mail items"""
        act = self._login(server, passwd)
        response, data = act.select('[Gmail]/All Mail')
        if response != 'OK':
            raise ValueError('ERROR getting list of mail items')
        logging.info('Logged in, processing items in mailbox to get your list...')
        act.close()
        logging.info('Total Count {}...'.format(data))
        return data
...
if __name__ == "__main__":
    ...
    email.get_mail_items(server,passwd)